...
- **frontend-new/** — the new platform's SPA (Vue 3 + Vite + Element Plus + Pinia + Vue Router). `npm run dev` serves on :5173 and proxies `/api` (prefix-stripped) + `/ws` (`ws:true`) to backend :8081 (backend has no CORS). Five views under `src/views/` (Login/Register/KnowledgeBase/Chat/AgentDesk/Dashboard); `src/api/stream.js` parses the `/chat/stream` SSE via fetch+ReadableStream; `src/composables/useTicketSocket.js` + `src/components/RealtimeSession.vue` drive the `/ws/chat` realtime session (shared by visitor & agent sides).
- **frontend/** — bundled `nginx-1.18.0` serving the legacy dianping SPA (reference only; superseded by `frontend-new/`).
